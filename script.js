    ...
    { 
        ...
        const createInteractiveImage = (src, isSecond = false) => {
            ...
            img.onload = () => {
                ...
                // Set a default scale to fit the image within the canvas without stretching
    ...
    const lineHeight = obj.size * 1.2 * obj.scale * s; // Added line height for spacing
    const startY = obj.y * s - ((lines.length - 1) * lineHeight) / 2; // Adjust start Y for vertical centering
    ...
    lines.forEach((line, index) => {
        ...
        const openModal = (type) => {
            ...
            else if (type === 'photo2') { [title, items, handler] = ['Photo 2', templates.photos2, (url) => createInteractiveImage(url, true)]; } // No upload button here
            ...
            items.forEach(item => {
                ...
                if (type === 'text') { div.className += ' text-template-item'; div.textContent = item.content; div.onclick = () => { handler(item.content); modal.style.display = 'none'; };
                } else { div.innerHTML = `<img src="${item.url}" alt="${item.name}">`; div.onclick = () => { handler(item.url); modal.style.display = 'none'; }; }
    ...
    if (obj.isText) {
        // Updated to correctly measure multi-line text
    ...
    return { x: 0, y: 0, width: 0, height: 0 };
        ...
        canvas.addEventListener('mousedown', handleStart); canvas.addEventListener('mousemove', handleMove); document.addEventListener('mouseup', handleEnd);
        canvas.addEventListener('touchstart', handleStart, { passive: false }); canvas.addEventListener('touchmove', handleMove, { passive: false }); document.addEventListener('touchend', handleEnd);
    });
